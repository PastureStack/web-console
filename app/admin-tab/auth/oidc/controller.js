...
export default Ember.Controller.extend({
  ...
  activateWithCodeFlow: function(code) {
    ...
    // Follow the established local-authentication activation sequence:
    // persist the provider while global security is disabled, exchange the
    // authorization code without the old provider cookie, and enable security
    // only after the new platform session exists.
    return model.save().then(() => {
      ...
      return this.restorePreviousProvider(rollbackModel).then(() => {
        if ( sessionSnapshot ) {
