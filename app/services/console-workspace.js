...
function sessionContext(instance, project, projectId) {
  ...
  let labels = value(instance, 'labels') || {};
  ...
  return {
    environmentTitle:
      ...
      projectId,
    stackTitle:
      value(instance, 'primaryStack.displayName') ||
      value(instance, 'primaryService.stack.displayName') ||
      labels[C.LABEL.STACK_NAME] ||
      null,
  };
}
...
export default Ember.Service.extend({
  ...
  init() {
    ...
    if (window.BroadcastChannel) {
      ...
      channel.onmessage = (event) => {
        let data = event.data || {};
        if (
          data.type === 'client-probe' &&
          data.clientId === this.get('clientId') &&
          this.get('clientReady')
        ) {
          channel.postMessage({
            type: 'client-present',
        ...
        if (
          data.type === 'client-present' &&
          data.clientId === this.get('clientId') &&
          data.nonce === this.get('clientProbeNonce')
        ) {
          this.set('clientConflict', true);
          return;
        }
        if (data.accountId === this.get('accountId')) {
          this.reloadSessions();
        }
      };
    ...
    this.prepareClientIdentity(!!storedClientId);
  ...
  prepareClientIdentity(hadStoredClientId) {
    let navigationType = workspaceNavigationType(window.performance);
    let channel = this.get('broadcastChannel');

    if (!hadStoredClientId || navigationType === 'reload') {
    ...
    if (!channel) {
      this.setProperties({
        clientId: workspaceClientId(window.sessionStorage, true),
        clientReady: true,
      });
      return;
    }

    let nonce = randomWorkspaceToken(18);
    this.setProperties({
      clientProbeNonce: nonce,
      clientConflict: false,
    });
    channel.postMessage({
      type: 'client-probe',
  ...
  openSession(kind, instance, options = {}) {
    ...
    if (existing) {
      ...
      this.updateEntryContext(existing, instance);
  ...
  projectForId(projectId) {
    let current = this.get('projects.current');
    if (current && current.get('id') === projectId) {
      return current;
    }
    let all = this.get('projects.all') || [];
    return all.find((project) => project.get('id') === projectId);
  },

  updateEntryContext(entry, instance) {
    let context = sessionContext(
      instance,
      this.projectForId(entry.get('projectId')),
      entry.get('projectId')
    );
    let changed =
      entry.get('environmentTitle') !== context.environmentTitle ||
      entry.get('stackTitle') !== context.stackTitle;
    if (changed) {
      entry.setProperties(context);
      this.saveSessions();
    }
  },
  ...
  decorateEntry(entry) {
    ...
    if (!entry.get('environmentTitle')) {
      entry.set('environmentTitle', entry.get('projectId'));
    }
    return entry;
  },
  ...
  loadInstance(entry) {
    ...
    return this.get('store').find(type, entry.get('instanceId')).then((loaded) => {
      if (!entry.isDestroyed) {
        ...
        this.updateEntryContext(entry, loaded);
      }
      return loaded;
    });
  },
  ...
  requestTermination(entry) {
    if (entry && entry.get('status') !== 'ended') {
      this.set('terminationEntry', entry);
    }
  },
