...
export function columnRoleFromName(name) {
  ...
  if ( /(^|\.)(cpurms|memoryrms|networkrms|storagerms)(\.|$)/.test(normalized) ) {
    return 'metric';
  }
...
/**
 * Keep a table at the smallest readable, non-wrapping width for its semantic
 * columns instead of stretching low-priority text across spare viewport space.
 */
export function fitWidths(widths, profiles = null) {
  if ( profiles && profiles.length === widths.length ) {
    return widths.map((width, index) => clampProfiledWidth(width, profiles[index]));
  }

  return widths.map((width) => clampInitialWidth(width));
}
...
TableColumnManager.prototype.autoFit = function(state) {
  ...
  state.widths = state.table.classList.contains('table-fit-content') ?
    fitWidths(natural, state.profiles) :
    distributeWidths(natural, hostAvailableWidth(state.table), state.profiles);
  state.manual = false;
  state.ready = true;
  this.applyWidths(state);
};
