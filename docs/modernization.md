...
The guarded host-storage bulk cleanup workflow is documented in
[`storage-cleanup.md`](storage-cleanup.md). Its release gate must continue to
prove that active mounts, recovery-marked names, and ambiguous detached items
cannot enter the automatic removal set.
