...
- state filters for all, active, detached, and removable rows;
...
selected. Names such as `backup`, `restore`, or `test` do not silently select
or exclude a row; the operator makes the final decision.
