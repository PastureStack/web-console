...
destination address when system-wide SMTP delivery is configured, and revoke
their own factors. An account never stores an SMTP server, sender identity, or
SMTP credentials.

The QR code is generated in the browser. Its provisioning secret is never sent
to an external image or QR-code service.

Registering a factor requires a recent one-use security confirmation and sends
a security notice without interrupting the current setup session. Regenerating
recovery codes, changing a recovery address, or removing factors revokes that
account's active sessions. The console requires sign-in again after any new
...
There is exactly one SMTP delivery configuration for the installation. It is
shared by every account and can be read or changed only by an active system
administrator. The selected account on the administration page does not scope
or duplicate these settings. Individual accounts store only their own verified
recovery destination address.

The system-wide settings include the server, port, sender, optional
credentials, connection and read timeouts, TLS mode, and code lifetime. Remote
SMTP must use STARTTLS or implicit TLS with hostname verification. Plain SMTP
is accepted only on loopback for an isolated fake-server test.
