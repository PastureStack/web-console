...
No automatic CI/CD, release publication, deployment, or production-readiness
claim is enabled. The manually dispatched validation workflow tests and builds
the exact selected commit on a GitHub-hosted runner and retains the reviewed
candidate for 30 days; it does not publish a release.
...
Host storage pages provide a guarded bulk cleanup workflow for explicit test
debris. Its eligibility rules, preview, concurrency limit, and protected-item
behavior are documented in [Storage cleanup](docs/storage-cleanup.md).
