...
test('it fits dense monitoring tables to their smallest semantic widths', function(assert) {
  let roles = ['state', 'name', 'metric', 'metric', 'metric', 'metric', 'image', 'ip', 'actions'];
  let profiles = roles.map((role) => profileForColumn(role));
  let widths = fitWidths([118, 330, 95, 128, 119, 106, 800, 126, 90], profiles);

  assert.deepEqual(widths, [118, 330, 112, 128, 119, 112, 480, 126, 90]);
  assert.equal(widths.reduce((total, width) => total + width, 0), 1615, 'does not stretch columns just to fill spare space');
});
