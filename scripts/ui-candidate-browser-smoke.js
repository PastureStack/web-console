...
// The shell harness validates the extracted candidate and makes it the process
// working directory before Node starts. Keeping the root out of the environment
// prevents an operator value from becoming a filesystem path inside this server.
...
function buildStaticAssetMap(resolvedRoot) {
  ...
  function visit(directory) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
        throw new Error(`static asset escaped UI_ROOT: ${absolute}`);
      }
  ...
  if (!index) throw new Error(`UI_ROOT does not contain index.html: ${resolvedRoot}`);
  return { assets, index };
}
...
function startServer() {
  ...
  const staticAssets = buildStaticAssetMap(resolvedRoot);
  const server = http.createServer((req, res) => {
    ...
    let assetKey;
    try {
      assetKey = decodeURIComponent(parsed.pathname.replace(/^\/+/, "")) || "index.html";
    } catch (error) {
    ...
    if (assetKey.includes("\0") || assetKey.includes("\\") || segments.includes("..")) {
      res.writeHead(403, { "content-type": "text/plain; charset=utf-8" });
      res.end("forbidden");
      return;
    }
    const asset = staticAssets.assets.get(assetKey) || staticAssets.index;
    res.writeHead(200, {
