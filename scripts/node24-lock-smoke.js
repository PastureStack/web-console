...
function expectYamljsBuildGraph() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pasturestack-yamljs-smoke-"));
  const file = path.join(tempDir, "fixture.yml");
  let loaded;
  try {
...
function expectLodashTemplateCompat() {
  expectVersion("lodash.template", "4.18.1-pasturestack.1");
  expectVersion("lodash-real", "4.18.1");

  const ownerVersions = {
    "ember-cli": "4.18.1-pasturestack.1",
  };
  for (const [owner, expectedVersion] of Object.entries(ownerVersions)) {
    ...
    if (template("hi <%= name %>")({ name: "pasture" }) !== "hi pasture") {
    ...
    if (template("<% var x = name.toUpperCase(); %><%= x %>")({ name: "pasture" }) !== "PASTURE") {
      fail(`${owner} lodash.template evaluation changed`);
    }
  }

  console.log("lodash-template-compat-smoke-ok wrapper=4.18.1-pasturestack.1 lodash-real=4.18.1 nested_build_only=0");
}
...
function expectCoreJsCompat() {
  ...
  expectMissing("babel-runtime");
  expectMissing("babel-register");
  console.log("core-js-compat-smoke-ok wrapper=2.6.13-rc16.0 core-js-pure=3.49.0 babel6_runtime=absent");
}
...
function expectEmberApiStoreFetchUpgrade() {
  ...
  const expectedApiStoreDependencies = {
  ...
  if (JSON.stringify(emberFetchInfo.dependencies || {}) !== JSON.stringify({ "ember-cli-babel": "8.3.1" })) {
    fail("ember-fetch native compatibility package has an unexpected build boundary");
  }
  expectVersion("ember-auto-import", "2.13.1");
  expectMissing("babel-traverse");
  expectMissing("babel-core");
  expectMissing("broccoli-rollup");
  expectMissing("rollup-plugin-babel");
  expectMissing("ember-route-action-helper");
  ...
  const fetchRuntime = fs.readFileSync(fetchRuntimePath, "utf8");
  for (const marker of [
    ...
    if (!fetchRuntime.includes(marker)) {
      fail(`ember-fetch native compatibility marker missing: ${marker}`);
    }
  }
  for (const forbidden of [
    ...
    if (fetchRuntime.includes(forbidden)) {
      fail(`ember-fetch native compatibility runtime contains obsolete marker: ${forbidden}`);
    }
  }
  ...
  vm.runInNewContext(fetchRuntime, sandbox, { filename: fetchRuntimePath });
